...
{{-- <div class="flex justify-between py-4 px-4 gap-y-3">
            <div class="2xl:grid 2xl:grid-rows-2 2xl:gap-[0] 2xl:mx-0 gap-[7.3rem]">
                @if ($webInfo->contact->phone->display == true)
                    ...
                    @endphp
                    <a href="tel:{{ $webInfo->contact->phone->value }}">
                        <p class="text-white sm:text-2xl text-md text-[16px]">{{ $formatPhone }}</p>
                    </a>
                @endif
            </div>
            ...
            @if (Request::is('/') || Request::is('prepaid_sim'))
                <div
                    class="  md:flex lg:flex 2xl:flex md:justify-between lg:justify-between 2xl:justify-between gap-6 items-left grid grid-rows-3 gap-y-2">
                    @foreach ($menu_footer as $footer)
                        <a href="/{{ $footer->cate_url }}">
                            <p class="text-white sm:text-lg text-md 2xl:text-[20px] text-[16px] text-center">
                                {{ $footer->cate_title }}</p>
                        </a>
                    @endforeach
                </div>
                ...
                <div class="s2xl:grid 2xl:grid-rows-2 2xl:gap-[0] 2xl:mx-0 gap-[7.3rem] ">
                    @if ($webInfo->contact->phone_true->display == true)
                        ...
                        <a href="tel:{{ $webInfo->contact->phone_true->value }}">
                            <p class="text-white sm:text-2xl text-[16px] text-right">
                                {{ $webInfo->contact->phone_true->value }}</p>
                        </a>
                    @endif
                </div>
        ...
        <div class="flex sm:justify-between items-center max-md:flex-col gap-y-2 px-3">
            <p class="text-white opacity-50 py-2 2xl:text-[16px] md:text-[16px] text-[14px] text-center">
                {{ $webInfo->footer->copy_right->value }}</p>
            @php
