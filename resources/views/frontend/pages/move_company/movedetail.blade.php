            ...
            <div class="w-[1536px] max-2xl:max-w-[90%] grid grid-cols-1 xl:grid-cols-3  gap-4 m-3">
                ...
                <div class="bg-white w-full 2xl:h-[500px]  items-center px-4 lg:px-1 mb-6 relative">
                    ...
                    <div id="slide-wrapper" class="flex justify-center items-center">
                        ...
                        <div id="slider" class="flex gap-4 overflow-x-hidden mx-4 ">
                            <img src="/{{ $move_product->thumbnail_link }}" alt=""
                            ...
                            @foreach ($moveRelates as $relate)
                                @if ($relate->id != $move_product->id)
                                    <img src="/{{ $relate->thumbnail_link }}" alt=""
                ...
                <div class="col-span-2 space-y-6 text-left mx-3">
                    ...
                    <div
                        ...
                        <div id="box" data-option="{{ $move_product->id }}"
                            class="box border border-gray-500 active hover:border-gray-500 bg-[#F8F9FA] rounded-lg px-2 py-2 max-ex:h-[8rem] h-[9rem] max-xs:h-[9.5rem] cursor-pointer">
                            <div class="flex mb-2">
                                <img src="/{{ $move_product->thumbnail_link }}" alt=""
                                    class="w-20 max-ex:w-[70px] rounded-lg">
                            ...
                            <div class="flex items-center">
                                ...
                                <p class="text-xl 2xl:text-[2rem] font-bold ml-[3.5rem] max-ex:ml-[2.5rem] text-red-600">
                                    {{ number_format($move_product->discount > 0 ? $move_product->discount : $move_product->price) }}
                                </p>
                        ...
                        @foreach ($moveRelates as $relateOption)
                            @if ($relateOption->id != $move_product->id)
                                <div id="box" data-option="{{ $relateOption->id }}"
                                    class="box border border-gray-10 hover:border-gray-500 bg-[#F8F9FA] rounded-lg px-2 py-2 max-ex:h-[8rem] h-[9rem] max-xs:h-[9.5rem] cursor-pointer">
                                    <div class="flex mb-2 ">
                                        <img src="/{{ $relateOption->thumbnail_link }}" alt=""
                                            class="w-20 max-ex:w-[70px] rounded-lg">
                                        <p class="text-lg font-medium ml-3 ">{{ $relateOption->package_options }}</p>
                                    </div>
                                    <div class="flex items-center">
                                        <img src="/images/check-one.png" alt=""
                                            class="check-box w-10 max-ex:w-[35px] ">

                                        <p
                                            class="text-xl 2xl:text-[2rem] font-bold ml-[3.5rem] max-ex:ml-[2.5rem] text-red-600">
                                            {{ number_format($relateOption->discount > 0 ? $relateOption->discount : $relateOption->price) }}
                                        </p>
                                        <p class="2xl:text-lg font-medium ml-2">บาท</p>
                                    </div>
                                </div>
            ...
            <div class="mx-4 max-2xl:mx-2">
                <div class="flex">
                    <button id="btn-package"
                        class="py-2 px-4 text-white bg-gradient-to-r from-[#F6911D] to-[#ED4312] rounded-t-[10px] text-[16px] max-xs:text-[14px]">แพ็กเกจ</button>
                    <button id="btn-detailPackage"
                        class="py-2 px-6 text-white bg-[#838383] rounded-t-[10px] text-[16px] max-xs:text-[14px]">รายละเอียด
                        แพ็กเกจ</button>
                    <button id="btn-condition"
                        class="py-2 px-6 text-white bg-[#838383] rounded-t-[10px] text-[16px] max-xs:text-[14px]">เงื่อนไข</button>
                </div>
                {{-- content detail --}}
                <div id="box-package"
                                ...
                                @if ($pos->id == $item)
                                    <div class="grid grid-cols-[90px,1fr] gap-2 px-4 mb-2">
                                        <div class="border-[1px] rounded-lg border-orange-500 p-1 w-[80px] h-[80px]">
                                            <img class="w-full h-full object-contain" src="/{{ $pos->thumbnail_link }}"
                                                alt="">
                                        </div>
