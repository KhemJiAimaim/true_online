...
var WeekDaysFr = ["Dim", "Lun", "Mar", "Mer", "Jeu", "Ven", "Sam"]
var MonthFr = ["Jan","Fév","Mar","Avr","Mai","Juin","Juil","Août","Sep","Oct","Nov","Dec"]
...
const available_time = document.querySelector('#available_time').value.split(",")
const available_time_sunday = document.querySelector('#available_time_sunday').value.split(",")
...
function disableTimeSelect(){
    ...
    if(time_canbook.length > 0){
        ...
        const currentTime = ("0" + timenow.getHours()).slice(-2) + pathLanguage==='fr'?":":"h" + ("0" + timenow.getMinutes()).slice(-2)
        let oneTimeUse = 0
...
function isSunday(){
    var time_canbook = document.querySelector('.col-time')
    if(time_canbook){
        if(datetime_selected){
            let html = ""
            if(datetime_selected.getDay() == 0){
                var time = available_time_sunday
                for(let i = 0; i < time.length; i++){
                    if(time[i] != ""){
                        html += `<div class="time ${i===1?"active":""}" onclick="selectTime(this)" data-time="${time[i]}">${time[i]}</div>`
                    }
                }
            } else {
                var time = available_time
                for(let i = 0; i < time.length; i++){
                    if(time[i] != ""){
                        html += `<div class="time ${i===1?"active":""}" onclick="selectTime(this)" data-time="${time[i]}">${time[i]}</div>`
                    }
...
function CalendarControl() {
    ...
    const calendarControl = {
        ...
        calWeekDays: pathLanguage=="fr"?WeekDaysFr:WeekDaysEn,
        calMonthName: pathLanguage=="fr"?MonthFr:MonthEn,
        ...
        checkDayIsDisabled: function(date) {
            return disable_by_day.indexOf(',' + WeekDaysEn[date-1].toLowerCase() + ',') >= 0
        },
