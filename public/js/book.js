...
if(btn_booking){
    btn_booking.addEventListener('click', function(){
        ...
        if(time_selected.indexOf('.') >= 0){
            time.setUTCHours(time_selected.split('.')[0], time_selected.split('.')[1], 00)
        } else if(time_selected.indexOf(':') >= 0){
            time.setUTCHours(time_selected.split(':')[0], time_selected.split(':')[1], 00)
        } else if(time_selected.indexOf('h') >= 0){
            time.setUTCHours(time_selected.split('h')[0], time_selected.split('h')[1], 00)
        } else {
            Swal.fire({
                title: "can't reserve.",
                icon: 'info',
