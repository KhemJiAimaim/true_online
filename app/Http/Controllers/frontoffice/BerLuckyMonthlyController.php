	...
	public function get_product_all(Request $request)
	{
			...
			if($getpost['cate'] != 3) {
				...
				$sql = "SELECT *,(SELECT thumbnail FROM bernetworks WHERE berproduct_alovers.product_network = bernetworks.network_name AND berproduct_alovers.monthly_status = bernetworks.monthly) as thumbnail, MID(product_phone, 4, 7) AS pp
								...
								WHERE product_sold = 'no' $getpost[sql]  AND category = 3
			...
			$totalCount = !empty($result) ? $result[0]->total_count : 0;
			...
			if($getpost['cate'] == 3 && !empty($berproducts)) {
				$firstProduct = $berproducts[0];
				$lastProduct = end($berproducts);
				$group_first = array_filter($berproducts, function($item) use ($firstProduct) {
						return $item->lover_group == $firstProduct->lover_group;
				});
				$group_last = array_filter($berproducts, function($item) use ($lastProduct) {
						return $item->lover_group == $lastProduct->lover_group;
				});
		
				$value = [];
				if(count($group_first) < 2) {
						$value[] = $group_first[0]->lover_group;
						array_shift($berproducts);
				}
				if (count($group_last) < 2) {
						$lastItem = end($group_last); // ดึงสมาชิกสุดท้ายของ $group_last
						$value[] = $lastItem->lover_group;
						array_pop($berproducts);
				}
				if($value) {
						$berlove = DB::select("
								SELECT *, 
								(SELECT thumbnail FROM bernetworks WHERE berproduct_alovers.product_network = bernetworks.network_name AND berproduct_alovers.monthly_status = bernetworks.monthly) as thumbnail, 
								MID(product_phone, 4, 7) AS pp
								FROM berproduct_alovers 
								WHERE lover_group IN (?)", [implode(",",$value)]
						);
						$berproducts = array_merge($berproducts, $berlove);
				}
				// dd($berlove);
			}
